...
const calculateCompatibility = (date1: string, time1: string, date2: string, time2: string): { score: number, analysis1: string, analysis2: string, compatibilityAnalysis: string } => {
  const w1 = 0.25;
  const w2 = 0.25;
  const w3 = 0.25;
  const w4 = 0.25;
  ...
  const commonYears = Math.min(currentYear - birth1.getUTCFullYear(), currentYear - birth2.getUTCFullYear());
  const commonYearsMatch = Math.min(100, commonYears * 5);

  const matchingPercentage = w1 * ageMatch + w2 * zodiacMatch + w3 * timeMatch + w4 * commonYearsMatch;
  return {
...
const ResultDisplay: React.FC = () => {
  ...
  const { date1, date2, time1 = '00:00', time2 = '00:00' } = router.query;
  const [result, setResult] = useState<{ score: number, analysis1: string, analysis2: string, compatibilityAnalysis: string } | null>(null);

  useEffect(() => {
    if (date1 && date2) {
      const result = calculateCompatibility(date1 as string, time1 as string, date2 as string, time2 as string);
      setResult(result);
    }
